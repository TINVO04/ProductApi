...
Request DTO không chứa `Id`; server chịu trách nhiệm sinh ID khi tạo mới và lấy ID từ route khi cập nhật. Response DTO chứa `Id` để client nhận diện tài nguyên. Model `Product` chỉ được sử dụng nội bộ trong controller và được mapping thủ công sang `ProductResponseDto` trước khi trả response.
...
File `src/ProductApi/ProductApi.http` chứa request mẫu cho Day 1, Day 2 và Day 3:

- `GET /api/health` và `GET /api/info`.
- GET danh sách, tìm kiếm, chi tiết thành công và chi tiết `404`.
- POST tạo sản phẩm hợp lệ với `201 Created`.
- POST kiểm tra name quá ngắn, price bằng `0` và quantity âm với `400 Bad Request`.
- PUT cập nhật sản phẩm thành công với `200 OK`.
- PUT kiểm tra ID không tồn tại với `404 Not Found`.
- PUT kiểm tra price âm với `400 Bad Request`.

Có thể gửi từng request trực tiếp trong IDE hỗ trợ file `.http`. Các trường hợp Day 3 đã được xác minh với response `200`, `201`, `400` và `404`.
